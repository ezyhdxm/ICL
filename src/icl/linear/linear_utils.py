...
def compare_check_point_task_vector_to_final(
    ...
    """
    final_task_vectors: Tensor of shape (n_tasks, n_embd)
    """
    # Load checkpoint
    checkpoint = torch.load(checkpoint_path, map_location=config.device)
...
def load_checkpoint(exp_name, checkpoint_file):
    ...
    Args:
        ...
        step (int): Step number of the checkpoint to load.
    Returns:
        model (torch.nn.Module): The model loaded from the checkpoint.
        config (ConfigDict): The configuration used for the model.
    """
    exp_dir = get_path_to_exp_dir(exp_name)
...
def plot_checkpoint_diffs(exp_name, layer_index, tasks=None, title=None, yaxis_title="Mean difference"):
    """
    - tasks: list of task indices to plot; default = all
    """
