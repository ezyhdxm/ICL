import torch
from icl.tasks.base_latent_abc import BaseLatentSequenceTask
...
class ReversedTask(BaseLatentSequenceTask):
    def _init_task_pool(self):
        max_length = torch.max(self.repeat_length).item()
        task_pool = torch.randint(high=self.num_states, 
                                  size=(self.total_trans, max_length), 
                                  device=self.device)
    ...
    def _get_index(self, hidden_state, length):
        ...
        total = 2*length + 1
        x[x > length] = total[x > length].long() - x[x > length]
        return x - 1
    ...
    def _hidden_state_update(self, x: torch.Tensor, length: torch.Tensor) -> torch.Tensor:
        ...
        x[replace_mask] = L[replace_mask]+1
    ...
    def _get_total_length(self):
        return torch.max(self.repeat_length)
