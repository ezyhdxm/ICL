...
def get_loss_plots(config, train_results, folder="loss_plots", show=False, verbose=False):
    ...
    # Extract losses
    train_losses = log["train/task_loss"] if len(log["train/task_loss"]) > 0 else log["train/loss"]
    ...
    loss_path = os.path.join(folder, f"loss_{datetime.now().strftime('%Y%m%d_%H%M')}.png")
    plt.tight_layout()
...
def get_loss_plots(config, train_results, folder="loss_plots", show=False, verbose=False):
    ...
    if len(eval_accs) >= 1:
        axes[0].plot(eval_steps, eval_accs, linestyle='--', color='lightgreen', label='Validation Accuracy')
        axes[1].plot(eval_steps, eval_accs, linestyle='--', color='lightgreen', label='Validation Accuracy')
    ...
    if len(ood_accs) >= 1:
        axes[0].plot(eval_steps, ood_accs, linestyle='--', label='OOD Accuracy')
        axes[1].plot(eval_steps, ood_accs, linestyle='--', label='OOD Accuracy')
