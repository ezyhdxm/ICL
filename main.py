...
from task import DataArgs, Dataset, iterate_batches
...
if __name__ == '__main__':
    ...
    full_range_toks = torch.from_numpy(np.arange(ds.vocab_size)).cuda()
    wk1_range_toks = full_range_toks.clone()

    conds = torch.from_numpy(np.array(ds.cond)).cuda()
    used_idxs = np.arange(ds.vocab_size)
    ...
    for i, (x, y, outs) in enumerate(iterate_batches(ds, batch_size=cfg.optim_args.batch_size,
        ...
        if cfg.max_iters is not None and i >= cfg.max_iters:
            if cfg.save_dir is not None:
                ...
                pickle.dump(
                    {'x': x_exp, 'out': out_exp, 'scores': scores, 'preds': preds}, open(outdir / 'exp.pkl', 'wb')
                    )
            sys.exit(0)
        ...
        if i % cfg.eval_delta == 0:
            if cfg.data_args.num_spe_toks > 0:
                acc_tot = (pred.argmax(-1)[outs >= 1] == y[outs >= 1]).float().mean().item()
                sl = 10
                acc_start = (pred[:,:sl].argmax(-1)[outs[:,:sl] >= 1] == y[:,:sl][outs[:,:sl] >= 1]
                             ).float().mean().item()
                el = 500
                acc_end = (pred[:,-el:].argmax(-1)[outs[:,-el:] >= 2] == y[:,-el:][outs[:,-el:] >= 2]
                           ).float().mean().item()
                ...
                if cfg.model_args.n_layers >= 2:
                    if cfg.model_args.no_ffn:
                        ...
                        ff1_loss = test_ff1(model, full_range_toks, conds, used_idxs)
                    wk0_acc = test_wk0(model, range_pos_toks, cfg)
                    wk0_64_acc = test_wk0(model, range_pos_toks, cfg, cutoff=64)
                    wk1_acc = test_wk1(model, wk1_range_toks, range_toks)
                    wo1_acc = test_wo1(model, range_toks)
                else:
                    ff1_loss = -1
                ...
                with torch.no_grad():
                    pred_t = model(x_t)
                acc_end_test = (
                    pred_t[:,-el:].argmax(-1)[outs_t[:,-el:] >= 1] == y_t[:,-el:][outs_t[:,-el:] >= 1]
                    ).float().mean().item()

                logging.info(f'{i} (
                             {dt_data:.2f}, {dt:.2f}, 
                             {t - t0:.2f}): {loss.item():.4f} 
                             ({loss_bigram:.4f} / {loss_head:.4f}), 
                             {acc_tot:.4f} ({acc_start:.4f} / {acc_end:.4f} / {acc_end_test:.4f}, 
                             {avg_outs:.2f})'
                             )
                if cfg.log_probes:
                    logging.info(
                        f'accs wk0: {wk0_acc:.4f} ({wk0_64_acc:.4f}), 
                        wk1: {wk1_acc:.4f}, wo1: {wo1_acc:.4f}, ff1: {ff1_loss:.4f}'
                        )
